...
def extract_movie_details(movie_link):
    ...
    if movie_soup is not None:
        ...
        rating = (
            movie_soup.find("span", attrs={"class": "sc-bde20123-1 iZlgcd"}).get_text()
            if movie_soup.find("span", attrs={"class": "sc-bde20123-1 iZlgcd"})
            else None
        )

        plot_text = (
            movie_soup.find("span", attrs={"class": "sc-6a7933c5-1 fPmRoa"})
            .get_text()
            .strip()
            if movie_soup.find("span", attrs={"class": "sc-6a7933c5-1 fPmRoa"})
            else None
        )
